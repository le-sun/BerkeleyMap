...
public class MapServer {
    ...
    private static LinkedList<Long> finishedRoute = new LinkedList<>();
    ...
    public static Map<String, Object> getMapRaster(Map<String, Double> params, OutputStream os) {
        ...
        QuadTree tree = new QuadTree("root", ROOT_ULLON, ROOT_ULLAT, ROOT_LRLON, ROOT_LRLAT);
        double ullon = params.get("ullon"); double lrlon = params.get("lrlon");
        double ullat = params.get("ullat"); double lrlat = params.get("lrlat");
        double longDifference = Math.abs(ullon) - Math.abs(lrlon);
        double latDifference = Math.abs(params.get("ullat")) - Math.abs(params.get("lrlat"));
        double box = longDifference * 288200;
        double feetPerPixel = box / params.get("w");
        LinkedList<Node> tiles = new LinkedList<Node>();
        for (Node n : tree.children()) {
            tree.tI(n, ullon, ullat, lrlon, lrlat, feetPerPixel, tiles);
        }
        tiles = tree.orderTiles(tiles); tiles = reverse(tiles); int height = 0;
        Node firstNode = tiles.getFirst();
        for (Node n : tiles) {
            if (firstNode.getTopX() == n.getTopX()) {
                height += 1;
            } else {
                break;
            }
        }
        int width = tiles.size() / height; rW = width * TILE_SIZE; rH = height * TILE_SIZE;
        putParams(rasteredImageParams, tiles);
        BufferedImage result = new BufferedImage(rW, rH, BufferedImage.TYPE_INT_RGB);
        Graphics graphics = result.getGraphics();
        int x = 0; int y = 0;
        for (Node n : tiles) {
            String filename = IMG_ROOT + n.toString() + ".png";
            try {
        ...
        Stroke line = new BasicStroke(MapServer.ROUTE_STROKE_WIDTH_PX,
                BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
        ((Graphics2D) graphics).setStroke(line);
        graphics.setColor(ROUTE_STROKE_COLOR);
        for (int i = 0; i < finishedRoute.size() - 1; i++) {
            String currentID; String nextID;
            GraphNode current; GraphNode next;
            if ((i + 1) >= finishedRoute.size()) {
                nextID = finishedRoute.get(i).toString();
                next = (GraphNode) g.getGraphNodes().get(nextID);
                current = next;
            } else {
                nextID = finishedRoute.get(i).toString();
                currentID = finishedRoute.get(i + 1).toString();
                current = (GraphNode) g.getGraphNodes().get(currentID);
                next = (GraphNode) g.getGraphNodes().get(nextID);
            }
            double rLon = (double) rasteredImageParams.get("raster_ul_lon");
            double rLat = (double) rasteredImageParams.get("raster_ul_lat");
            double rLLon = (double) rasteredImageParams.get("raster_lr_lon");
            double rLLat = (double) rasteredImageParams.get("raster_lr_lat");
            double ddpX = (rLLon - rLon) / rW;
            double ddpY = (rLat - rLLat) / rH;

            int x1 = (int) ((current.getLon() - rLon) / ddpX);
            int y1 = (int) ((rLat - current.getLat()) / ddpY);
            int x2 = (int) ((next.getLon() - rLon) / ddpX);
            int y2 = (int) ((rLat - next.getLat()) / ddpY);
    ...
    private static void putParams(HashMap params, LinkedList<Node> tiles) {
        params.put("raster_ul_lon", tiles.getFirst().getTopX());
        params.put("raster_ul_lat", tiles.getFirst().getTopY());
        params.put("raster_lr_lon", tiles.getLast().getBottomX());
        params.put("raster_lr_lat", tiles.getLast().getBottomY());
        params.put("depth", tiles.getFirst().getHeight());
        params.put("raster_width", rW);
        params.put("raster_height", rH);
        params.put("query_success", true);
    }

    private static LinkedList<Node> reverse(LinkedList<Node> tiles) {
        LinkedList<Node> orderedTiles = new LinkedList<Node>();
        while (!tiles.isEmpty()) {
            orderedTiles.add(tiles.removeLast());
        }
        return orderedTiles;
    }
    ...
    public static LinkedList<Long> findAndSetRoute(Map<String, Double> params) {
        ...
        AStarPathFinding route =
                new AStarPathFinding(startLon, startLat, endLon, endLat, g.getGraphNodes());
        LinkedList<Long> completedRoute = route.solve();
        clearRoute();
        finishedRoute = completedRoute;
    ...
    /**
     * In linear time, collect all the names of OSM locations that
     * prefix-match the query string.
     *
     * @param prefix Prefix string to be searched for. Could be any case, with our without
     *               punctuation.
     * @return A <code>List</code> of the full names of locations
     * whose cleaned name matches the
